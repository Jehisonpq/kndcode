...
const allScroll = () => {
    ...
    ScrollReveal().reveal('.section-left .title-section', { reset: true });
...
const changeImage = (id1, id2) => {
    ...
    $(window).scroll(function() {
        ...
        if((bottom_of_screen > top_element) && (top_of_screen < bottom_of_element)){
            ...
            for(const seccion in secciones) {
                if(secciones[seccion].id === id1.split("#")[1]) {
                    document.getElementById('scroll-link').setAttribute('href', '#' + secciones[parseInt(seccion) + 1].id)
                }
            }
        }
        else {
        ...
        console.log($('.container-app').innerHeight() - $('#section3').outerHeight())

        if($(window).scrollTop() >= $('.container-app').innerHeight() - $('#section3').outerHeight() -400) {
